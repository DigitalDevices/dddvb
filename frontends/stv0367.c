...
static struct st_register def0367ter[STV0367TER_NBREGS] = {
	...
	{R367TER_TSSTATEM,	0xb0}, {R367TER_TSSTATEL,	0x40},
	{R367TER_TSCFGH,	0x70},
...
int stv0367ter_init(struct dvb_frontend *fe)
{
	...
	stv0367_writereg(state, R367TER_I2CRPT, 0x38);
				...
				struct dvb_frontend_parameters *param)
{
	...
	temp = (int)
		((InternalFreq - state->IF) * (1 << 16)	/ (InternalFreq));
				...
				struct dvb_frontend_parameters *param)
{
	...
	*/
	...
	stv0367cab_set_derot_freq(state, cab_state->adc_clk,
		(1000 * (s32)state->IF + cab_state->derot_offset));
...
/* not clear for me */
		if (state->IF != 0) {
			if (state->IF > cab_state->adc_clk / 1000) {
				cab_state->freq_khz =
					FE_Cab_TunerGetFrequency(pIntParams->hTuner)
				- stv0367cab_get_derot_freq(state, cab_state->adc_clk)
				- cab_state->adc_clk / 1000 + state->IF;
			} else {
				cab_state->freq_khz =
						FE_Cab_TunerGetFrequency(pIntParams->hTuner)
						- stv0367cab_get_derot_freq(state, cab_state->adc_clk)
